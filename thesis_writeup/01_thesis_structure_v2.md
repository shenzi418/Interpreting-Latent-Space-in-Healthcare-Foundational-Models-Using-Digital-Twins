...
- 4.3.2 Track 3: the pre-registered rule quoted verbatim from the dated May log (**POSITIVE 4-class ≥ 0.45, p<0.01; PARTIAL 2-class ≥ 0.55, p<0.01**; May scoring NEGATIVE at 0.235 / 0.461 on pre-fix encoders); re-scored on `exp8_leadfix_medalonly`, n=4324: 4-class **0.3440 (target) / 0.3357 (strict) — POSITIVE not met**; 2-class **0.6299 / 0.6521 — met under both scalers at the dated 0.55 bar** (footnote: a stricter 0.65 bar appears in later working notes and would be met under strict only). Permutation p at the 1e-4 floor for both arms on both endpoints → uninformative; only paired tests order arms. **Table 4.5** verdict table (both bars shown, both scalers).
...
|---|---|---|---|
| N1 | **Track 3 PARTIAL bar: 0.55 (dated May pre-registration) vs 0.65 (undated memory + Aug docs)** | Quote the dated May rule (0.55) verbatim; report "met under both scalers"; footnote the 0.65 | Pre-registration integrity; changes the frozen wording in CLAUDE.md #7 and Q1(a) in the Marta email — **you may want to add a line to Marta** |
...
| N8 | **"strict" is two different scalers**: P1's strict = `target_pool_measured` (target-side, un-imputed pool) vs P2's strict = `source` (no target info). Q1 in the Marta email lists (a) Track 3 PARTIAL "under strict" and (c) F2 "under strict/source" as if they were the same axis | name modes by code name everywhere; re-phrase Q1 for Marta as *two* choices: P1 (legacy `target` vs `target_pool_measured`) and P2 (`source` vs `target`) | otherwise the thesis (and Marta) will read Track 3's "strict" as "no target information", which is false |
