...
| **Em dashes** as the default connective (my own habit) | Target ≤ 1 per 200 words. Prefer a full stop, a comma, or a colon. Keep the dash for a genuine interruption. |
| **Parenthetical asides and bracketed cross-references** (owner rules, 2026-08-25 and 2026-08-27) | Give an example or qualification its own short sentence, or attach it with "which means" / "for X this means". Weave section references into the sentence — "reported in §4.2", "Section 4.3 scores this" — instead of appending "(§4.2)". Parentheses stay only for citations, statistics and units. |
| **Cross-section repetition** (owner rule, 2026-08-27): restating content or reusing the phrasing of an earlier section (e.g. a chapter overview echoing the Introduction) | Each fact and each formula-phrase appears once at full strength; later mentions are shorter, differently worded, or a plain pointer. Overviews state what is new in their chapter, not what Chapter 1 already said. |
